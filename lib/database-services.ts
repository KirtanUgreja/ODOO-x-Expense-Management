...
import type { User, Company, Expense, ApprovalRecord, EmailNotification } from './types'
...
export class ExpenseService {
  static async create(expenseData: Omit<Expense, 'id' | 'createdAt' | 'status' | 'currentApprovalStep' | 'approvalHistory'>): Promise<Expense> {
    const expense = await prisma.expense.create({
      data: {
        ...
        ocrData: expenseData.ocrData ? JSON.stringify(expenseData.ocrData) : undefined,
        approvalHistory: {
          create: []
        }
      },
    ...
    return {
      ...
      ocrData: expense.ocrData ? JSON.parse(expense.ocrData.toString()) : undefined
    }
  }
  ...
  static async findById(id: string): Promise<Expense | null> {
    ...
    return {
      ...
      companyId: expense.companyId,
      currentApprovalStep: expense.currentApprovalStep,
      approvalHistory: expense.approvalHistory.map((record: any) => ({
...
export class EmailNotificationService {
  static async create(notification: Omit<EmailNotification, 'id' | 'timestamp'>): Promise<EmailNotification> {
    ...
    return {
      ...
      type: emailNotification.type as any,
